...
def main():
  # Welcome
  print("Welcome to the solution of Challenge number 2")
  ...
  for id in range(nPoints):
    # Wear mask
    tempArr[0:5] = pointList[id][:]
    tempArr[6]   = 1

    # Collect all ill points
    if (pointList[id][5] == 1):
      illPointList.append(tempArr)
    else:
      finePointList.append(tempArr)
  ...
  for time in range(ntimestep):
    ...
    # Third step: check near-contact with ill points
    for id1 in range(numIllPoint):
      for id2 in range(numFinePoint):
    ...
    # # Export data++++++++++++++++++++++++++++++++++++++++++++++++
    # # Write ill points
    # fileName = "./Task02/IllPoint_TimeStep_" + str(time)
    # with open(fileName,"w+") as my_csv:
    #   csvWriter = csv.writer(my_csv,delimiter = ' ')
    #   csvWriter.writerows(illPointList)

    # # Write fine points
    # fileName = "./Task02/FinePoint_TimeStep_" + str(time)
    # with open(fileName,"w+") as my_csv:
    #   csvWriter = csv.writer(my_csv,delimiter = ' ')
  ...
  # Print number of infected cases <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  print("***********************************************")
  numIllPoint = len(illPointList)
  numFinePoint = len(finePointList)
...
def contaminate_point(illPointList, id1, finePointList, id2):
  if (check_near_contact(illPointList, id1, finePointList, id2)):
    # Count mask use
    illPointList[id1][6] = illPointList[id1][6] + 1
    finePointList[id2][6] = finePointList[id2][6] + 1
    
    # Check mask expire
    if ((check_near_contact(illPointList, id1, finePointList, id2)) and
        (illPointList[id1][6] > maxMaskUse) and 
        (finePointList[id2][6] > maxMaskUse)):
